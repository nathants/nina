// choose provides AI-powered file selection tool as nina subcommand
// reads file paths from stdin and uses AI models to select relevant files
// based on user criteria embedded in the file contents
package choose
...
func init() {
	lib.Commands["choose"] = choose
	lib.Args["choose"] = chooseArgs{}
}
...
func choose() {
	...
	for {
		...
		if err != nil {
			if err == io.EOF {
				if line != "" {
					filePaths = append(filePaths, strings.TrimSpace(line))
				}
				break
			}
	...
	if len(filePaths) == 0 {
		fmt.Fprintf(os.Stderr, "Error: no file paths provided via stdin\n")
		os.Exit(1)
	}
	...
	gitRoot := getGitRoot()
	baseFilename := fmt.Sprintf("%s_choose_%s", timestamp, args.Model)

	// Create agents/choose directory with timestamp subdirectory
	sessionTimestamp := lib.GetSessionTimestamp()
	agentsDir := fmt.Sprintf("%s/agents/choose/%s", gitRoot, sessionTimestamp)
	err := os.MkdirAll(agentsDir, 0755)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error creating agents/choose directory: %v\n", err)
		// Fall back to /tmp with timestamp
		agentsDir = fmt.Sprintf("/tmp/choose-%s", sessionTimestamp)
		_ = os.MkdirAll(agentsDir, 0755)
		baseFilename = fmt.Sprintf("choose-%s-%s", args.Model, timestamp)
	}
	...
	err = runChoose(args.Model, prompt, !args.NoStream, !args.NoOAuth, args.Debug, agentsDir, baseFilename)
	if err != nil {
...
func runChoose(model, prompt string, stream bool, useOAuth bool, debug bool, agentsDir, baseFilename string) error {
	...
	response, err = callProvider(provider, modelID, prompt, stream, useOAuth)
	if err != nil {
		return err
	}
...
func callProvider(prov, modelID, message string, stream bool, useOAuth bool) (string, error) {
	...
	case "gemini":
		...
		return gemini.Handle(ctx, convertGeminiModelID(modelID), sysPrompt, messages, nil, reasoningCallback, false, thinkingBudget)
